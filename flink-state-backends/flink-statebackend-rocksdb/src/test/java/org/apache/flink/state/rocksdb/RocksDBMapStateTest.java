...
class RocksDBMapStateTest {
    ...
    // Fill the cache once, then exercise two refills, ending with a partial batch.
    private static final int MAP_ENTRY_COUNT = 2 * ITERATOR_CACHE_SIZE + 1;

    // A deleted entry can leave both its value and a tombstone.
    // Allow both records per entry, plus one extra skip.
    private static final int SKIPPABLE_INTERNAL_KEY_LIMIT = 2 * MAP_ENTRY_COUNT + 1;

    // An unbounded seek into the neighboring map must exceed the skip limit.
    private static final int NEIGHBOR_TOMBSTONE_COUNT = SKIPPABLE_INTERNAL_KEY_LIMIT + 1;
    ...
    void testIteratorCacheReloadDoesNotScanDeletedEntriesOfNextKey(boolean removeEntries)
        ...
        assertThat(keys)
                .containsExactlyElementsOf(
                        IntStream.range(0, MAP_ENTRY_COUNT).boxed().collect(Collectors.toList()));
    ...
    void testIteratorCacheReloadAfterEntriesWereRemoved(boolean removeLastReturnedEntry)
        ...
        for (int i = firstRemovedKey; i < MAP_ENTRY_COUNT; i++) {
            state.remove(i);
        }
    ...
    private void createNeighborTombstones() throws Exception {
        ...
        for (int i = 0; i < NEIGHBOR_TOMBSTONE_COUNT; i++) {
            neighbor.put(i, i);
        }
        flush();
        for (int i = 0; i < NEIGHBOR_TOMBSTONE_COUNT; i++) {
        ...
        assertThat(
                        backend.db.getPropertiesOfAllTables(columnFamily).values().stream()
                                ...
                                .sum())
                .isEqualTo(NEIGHBOR_TOMBSTONE_COUNT);
        mapState(2, 0).put(NEIGHBOR_TOMBSTONE_COUNT, NEIGHBOR_TOMBSTONE_COUNT);
    }
    ...
    private void assertNeighborPreserved() throws Exception {
        assertThat(mapState(2, 0).get(NEIGHBOR_TOMBSTONE_COUNT))
                .isEqualTo(NEIGHBOR_TOMBSTONE_COUNT);
    }
